...
const devRouter = [
  {
    ...
    children: [
      ...
      {
        path: '/SystemManage/MenuManage/List',
        title: '菜单管理'
      },
      {
        path: '/SystemManage/RoleManage/List',
        title: '角色管理'
      }
      // {
...
/**
 * 获取后端路由信息的 axios API，若需使用此方法需要后端建立专门的业务处理逻辑
 * @returns {Promise}
 */
const getRouterByUser = () => {
  // return Axios.post('/Base_Manage/Base_Action/GetMenuTreeList')
  return new Promise((resolve, reject) => {
    Axios.get('/myauth/web/getMenuList').then(resJson => {
      if (resJson.success) {
