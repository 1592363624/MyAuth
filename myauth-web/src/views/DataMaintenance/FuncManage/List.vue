<template>
  ...
  <a-card :bordered="false">
    ...
    >
      ...
      <a-button
        type="primary"
        @click="handleOpenRunJs(null,'函数调试')"
      >函数调试</a-button>
    </div>
    ...
    >
      ...
      >
        <template>
          <a @click="handleOpenRunJs(record.id,'运行测试')">运行测试</a>
          <a-divider type="vertical" />
          <a @click="handleEdit(null, record.id)">编辑</a>
          <a-divider type="vertical" />
    ...
    ></edit-form>
    <run-js-test ref="runJsTest"></run-js-test>
  </a-card>
  <!-- </a-modal> -->
</template>
...
export default {
  ...
  methods: {
    ...
    hanldleAdd (softId) {
      this.behaviorType = 1
      this.$refs.editForm.openForm(softId, null, '新增函数')
    },
    handleEdit (softId, id) {
      this.behaviorType = 2
      this.$refs.editForm.openForm(softId, id, '编辑函数')
    },
    handleDelete (id) {
      const thisObj = this
      this.$confirm({
    ...
    submitDelete (id, resolve, reject) {
      this.$http.post('/myauth/web/delJs', { id }).then(resJson => {
        resolve()
    ...
    handleOpenRunJs (id, title) {
      this.$refs.runJsTest.openForm(id, title)
    }
  }
}
</script>
