...
export default {
  ...
  methods: {
    ...
    hanldleAdd (softId) {
      this.behaviorType = 1
      this.$refs.editForm.openForm(null, softId, '新增回复')
    },
    handleEdit (id, softId) {
      this.behaviorType = 2
      this.$refs.editForm.openForm(id, softId, '编辑回复')
    },
    handleDelete (id) {
      const thisObj = this
      this.$confirm({
    ...
    submitDelete (id, resolve, reject) {
      this.$http.post('/myauth/web/delMsg', { id }).then(resJson => {
        resolve()
