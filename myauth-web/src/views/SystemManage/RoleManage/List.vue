...
export default {
  ...
  methods: {
    ...
    hanldleAdd () {
      this.behaviorType = 1
      this.$refs.editForm.openForm(null, '新增角色')
    },
    handleEdit (id) {
      this.behaviorType = 2
      this.$refs.editForm.openForm(id, '编辑角色')
    },
    handleDelete (id) {
      const thisObj = this
      this.$confirm({
    ...
    submitDelete (id, resolve, reject) {
      this.$http.post('/myauth/web/delRole', { id }).then(resJson => {
        resolve()
