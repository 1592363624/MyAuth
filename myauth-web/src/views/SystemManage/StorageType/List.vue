...
export default {
  ...
  methods: {
    ...
    getDataList () {
      ...
      this.loading = true
      this.$http
        .post('/myauth/web/getStorageTypeList', {
    ...
    hanldleAdd () {
      this.behaviorType = 1
      this.$refs.editForm.openForm(null, '新增额外存储类型')
    },
    handleEdit (id) {
      this.behaviorType = 2
      this.$refs.editForm.openForm(id, '编辑额外存储类型')
    },
    handleDelete (id) {
      const thisObj = this
      this.$confirm({
    ...
    submitDelete (id, resolve, reject) {
      this.$http.post('/myauth/web/delStorageType', { id }).then(resJson => {
        resolve()
