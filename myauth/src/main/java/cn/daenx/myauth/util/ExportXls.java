...
public class ExportXls {
     ...
     * @param list
     */
    public static void exportACard2Xls(HttpServletRequest request, HttpServletResponse response, String fileName, List<Acard> list) {
        request.getSession();
        response.setContentType("application/vnd.ms-excel");
        response.setCharacterEncoding("utf-8");
        try {
        ...
        //如果用这种方式导出，WPS或者office打开文件时会检测到是SYLK，即此文件是一个文本文件
        //这样没啥问题，但是如果第一个字符是"ID"，就会有弹窗警告提示，如果用小写的"id"或者其他字符就不会再有弹窗提示
        String ret = "id\t卡密\t面额\t生成时间\t使用时间\t使用人账号\t卡密状态";
        for (Acard acard : list) {
            ret = ret + "\n" + acard.getId() + "\t" + acard.getCkey() + "\t" + acard.getMoney() + "\t" +MyUtils.dateToStr(acard.getAddTimeName())+ "\t" +MyUtils.dateToStr(acard.getLetTimeName()) + "\t" + acard.getLetUser() + "\t" + acard.getStatusName();
        }
        OutputStream outputStream = null;
        try {
            outputStream = response.getOutputStream();
